...
import sys
import random
# MODIFIED: Import from the refactored mainconnect
from mainconnect_refactored import game
...
def run_league_match(team1, team2, is_rain_match, current_points, current_batting_info, current_bowling_info, web_mode=True):
    ...
    try:
        ...
        # MODIFIED: Pass write_to_file argument
        ...
        else: # Covers "cancelled" as well, but cancelled is handled before game call if rain hits.
            if winner_status != "cancelled": # only update W/L if not cancelled
