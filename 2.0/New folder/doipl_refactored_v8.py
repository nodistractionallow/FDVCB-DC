import os
import sys # Keep sys for sys.stdout manipulation if web_mode is False, otherwise can be removed.
import random
from mainconnect import game
from tabulate import tabulate # Keep for CLI mode (web_mode=False)
import copy

# REMOVED: dir_path management and initial cleanup of "scores" directory.
# This should be handled by the calling application or mainconnect.game if it stops file writing.

TEAMS = ['dc', 'csk', 'rcb', 'mi', 'kkr', 'pbks', 'rr', 'srh']

# REMOVED: battingf and bowlingf global variables.
...
def simulate_tournament_league_phase(team_list, web_mode=True):
    ...
    # REMOVED: global battingf, bowlingf access here. These vars are fully removed.
    ...
    for team1_sched, team2_sched in scheduled_matches:
        ...
        # REMOVED: battingf/bowlingf update
        if not web_mode:
...
if __name__ == "__main__":
    is_web_mode = True # MODIFIED: Default to True for web-focused behavior
    ...
    if not is_web_mode: # CLI-specific output for testing if needed
        ...
        # Using tabulate for CLI if available and web_mode is False
        # If tabulate is to be removed, this print needs to change.
        if tabulate:
            print(tabulate(final_table_data, headers="keys", tablefmt="grid"))
        else:
            for row in final_table_data: print(row)

        print("\nTop Players:")
        top_players = get_top_players(battingInfo, bowlingInfo)
        print("Batsmen:")
        if tabulate:
            print(tabulate(top_players["top_batsmen"], headers="keys", tablefmt="grid"))
        else:
            for p in top_players["top_batsmen"]: print(p)
        print("Bowlers:")
        if tabulate:
            print(tabulate(top_players["top_bowlers"], headers="keys", tablefmt="grid"))
        else:
            for p in top_players["top_bowlers"]: print(p)

        # REMOVED: batStats.txt and bowlStats.txt writing
        # REMOVED: print("bat", battingf, "bowl", bowlingf)
        # REMOVED: input("\nPress Enter to exit...")
    else:
        # For web mode, you would typically return or make available:
        # points, battingInfo, bowlingInfo, league_match_results, playoff_match_results, tournament_champion
        # For now, just a simple print to indicate web mode completion.
        print("Tournament simulation complete (web_mode=True). Data is in variables.")
        print(f"Winner: {tournament_champion}")
        # Example: print(league_match_results)
        # Example: print(get_points_table(points, TEAMS))
else:
    pass
