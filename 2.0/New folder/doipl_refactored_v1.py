...
# Note: display_points_table and display_top_players will need to accept points, battingInfo, bowlingInfo as arguments
# This will be refactored in a later step. For now, they will rely on the global scope within the if __name__ == "__main__" block.
...
def display_scorecard(bat_tracker, bowl_tracker, team_name, innings_num):
    ...
    for player_name_key in bat_tracker:
        ...
        elif how_out_status not in ['DNB', 'Did Not Bat']:
            sr_val = '-'
...
# Global definition for playoffs, will be moved into if __name__ == "__main__" later.
# Also needs refactoring to accept and return battingInfo, bowlingInfo
def playoffs(team1, team2, matchtag, current_batting_info, current_bowling_info): # Added info params
    ...
    try:
        ...
        for bat_map in ['innings1Battracker', 'innings2Battracker']:
            ...
            for player in tracker:
                if player not in current_batting_info: # Use passed in info
                    current_batting_info[player] = copy.deepcopy(tracker[player])
                    current_batting_info[player]['innings'] = 1
                    current_batting_info[player]['scoresArray'] = [int(tracker[player]['runs'])]
                else:
                    current_batting_info[player]['balls'] += tracker[player]['balls']
                    current_batting_info[player]['runs'] += tracker[player]['runs']
                    current_batting_info[player]['ballLog'] += tracker[player]['ballLog']
                    current_batting_info[player]['innings'] += 1
                    current_batting_info[player]['scoresArray'].append(int(tracker[player]['runs']))
        for bowl_map in ['innings1Bowltracker', 'innings2Bowltracker']:
            tracker = res[bowl_map]
            for player in tracker:
                if player not in current_bowling_info: # Use passed in info
                    current_bowling_info[player] = copy.deepcopy(tracker[player])
                    current_bowling_info[player]['matches'] = 1
                else:
                    current_bowling_info[player]['balls'] += tracker[player]['balls']
                    current_bowling_info[player]['runs'] += tracker[player]['runs']
                    current_bowling_info[player]['ballLog'] += tracker[player]['ballLog']
                    current_bowling_info[player]['wickets'] += tracker[player]['wickets']
                    current_bowling_info[player]['noballs'] = current_bowling_info[player].get('noballs', 0) + tracker[player].get('noballs', 0)
                    current_bowling_info[player]['matches'] += 1
        # display_points_table and display_top_players will be handled by the main orchestrator
        input("Press Enter to continue...")
        return winner, loser, current_batting_info, current_bowling_info # Return updated info
    except Exception as e:
        print(f"Error during {matchtag.upper()}: {str(e)}")
        input("Press Enter to continue or Ctrl+C to exit...")
        return team1, team2, current_batting_info, current_bowling_info # Default to team1 as winner
...
if __name__ == "__main__":
    ...
    for team1, team2 in scheduled_matches_final:
        ...
        try:
            ...
            for bowl_map in [('innings1Bowltracker',), ('innings2Bowltracker',)]:
                ...
                for player in bowl_tracker:
                    if player not in bowlingInfo:
                        ...
                        bowlingInfo[player]['noballs'] = bowlingInfo[player].get('noballs', 0) + bowl_tracker[player].get('noballs', 0)
                        bowlingInfo[player]['matches'] += 1
            ...
            points[team2]['P'] += 1
            if resList.get('superOverPlayed', False):
                teamA_actual_for_so = resList.get('innings1BatTeam')
                teamB_actual_for_so = resList.get('innings2BatTeam')
                ...
                elif team1 in points and team2 in points:
                     points[team1]['SO'] +=1
                     points[team2]['SO'] +=1
            if winner_status == "cancelled":
                ...
                points[teamA_actual]['ballsBowled'] += teamBBalls
                points[teamB_actual]['ballsBowled'] += teamABalls
            display_points_table(points) # Pass points
            display_top_players(battingInfo, bowlingInfo) # Pass battingInfo and bowlingInfo
    ...
    for team_name_key in TEAMS:
        ...
        nrr_val = 0
        if team_data['ballsFaced'] > 0 and team_data['ballsBowled'] > 0:
            nrr_val = (team_data['runsScored'] / team_data['ballsFaced']) * 6 - \
                      (team_data['runsConceded'] / team_data['ballsBowled']) * 6
        elif team_data['ballsFaced'] > 0:
            nrr_val = (team_data['runsScored'] / team_data['ballsFaced']) * 6
        elif team_data['ballsBowled'] > 0:
            nrr_val = - (team_data['runsConceded'] / team_data['ballsBowled']) * 6
    ...
    if len(pointsTabulate) >= 4:
        ...
        winnerQ1, loserQ1, battingInfo, bowlingInfo = playoffs(q1[0], q1[1], "Qualifier 1", battingInfo, bowlingInfo)
        finalists.append(winnerQ1)

        winnerElim, _, battingInfo, bowlingInfo = playoffs(elim[0], elim[1], "Eliminator", battingInfo, bowlingInfo)

        winnerQ2, _, battingInfo, bowlingInfo = playoffs(winnerElim, loserQ1, "Qualifier 2", battingInfo, bowlingInfo)
        ...
        if len(finalists) == 2:
            ...
            print("Could not determine two finalists for the Final match.")
    else:
        print("Not enough teams qualified for playoffs.")
    ...
    with open(os.path.join(dir_path, "batStats.txt"), "w") as f: # This will be removed later
        sys.stdout = f
        print(tabulate(battingTabulate, headers=["Player", "Innings", "Runs", "Average", "Highest", "SR", "Balls"], tablefmt="grid"))
        sys.stdout = sys.__stdout__
    with open(os.path.join(dir_path, "bowlStats.txt"), "w") as f: # This will be removed later
        sys.stdout = f
        print(tabulate(bowlingTabulate, headers=["Player", "Wickets", "Overs", "Runs Conceded", "NB", "Economy"], tablefmt="grid"))
        sys.stdout = sys.__stdout__

    print("bat", battingf, "bowl", bowlingf) # This will be removed or refactored
    input("\nPress Enter to exit...") # This will be removed
else:
    # This block can be used for testing functions individually if needed
    # For example:
    # test_points, test_bat, test_bowl = initialize_tournament_data(TEAMS)
    # print(test_points)
    pass
