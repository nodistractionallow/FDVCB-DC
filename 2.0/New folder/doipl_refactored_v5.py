...
# REMOVED display_scorecard function

def get_match_commentary(innings_log): # MODIFIED: Was display_ball_by_ball
    commentary_list = [] # MODIFIED: Accumulate commentary here
    # Removed parameters: innings_num, team_name, runs, balls, wickets, bat_tracker, bowl_tracker
    # These were mostly for display and are not needed for returning processed commentary_lines
    # or can be derived from innings_log if structured commentary objects were to be returned.

    # Example: innings_num = innings_log[0].get('inning', 0) if innings_log else 0
    # (Assuming 'inning' key exists in log, otherwise needs to be passed or handled)
    ...
    while i < len(innings_log):
        ...
        # Ensure commentary_lines is accessible, assuming it's global or passed in if this becomes a static method/class
        ...
        # MODIFIED: Append processed commentary to list instead of printing
        commentary_list.append(f"Ball {event_data.get('balls', 'N/A')}: {event_text} - {current_commentary_line}")
        i += 1

    # MODIFIED: Removed overall innings summary print and call to display_scorecard
    # An innings summary string could be added to commentary_list if needed.
    # For example:
    # innings_total_runs = sum(e.get('runs_this_ball',0) for e in innings_log if e.get('type') not in ['WIDE', 'NO_BALL_CALL']) # crude sum
    # innings_wickets = max([e['wickets'] for e in innings_log if 'wickets' in e], default=0) # crude wickets
    # last_ball_event = innings_log[-1] if innings_log else {}
    # innings_overs_balls = last_ball_event.get('balls', '0.0')
    # innings_summary_comment = f"\nInnings Total: {innings_total_runs}/{innings_wickets} in {innings_overs_balls} overs"
    # commentary_list.append(innings_summary_comment)
    # commentary_list.append(random.choice(commentary_lines['innings_end']))

    return commentary_list


def playoffs(team1, team2, matchtag, current_batting_info, current_bowling_info):
    print(f"\n{matchtag.upper()} - {team1.upper()} vs {team2.upper()}") # This print is for CLI, will be removed for web
    try:
        input("Press Enter to start the playoff match...") # To be removed for web
        print(random.choice(commentary_lines['start'])) # To be removed for web / handled by commentary

        res = game(False, team1.lower(), team2.lower(), matchtag) # game() needs to not print for web.

        # MODIFIED: Process commentary for web, rather than printing directly
        innings1_commentary = get_match_commentary(res['innings1Log'])
        innings2_commentary = get_match_commentary(res['innings2Log'])

        # For now, printing a snippet of commentary for verification (will be removed)
        print("Sample Innings 1 Commentary (first 3 lines):")
        for line in innings1_commentary[:3]: print(line)
        print("Sample Innings 2 Commentary (first 3 lines):")
        for line in innings2_commentary[:3]: print(line)

        if res.get('superOverPlayed', False) and res.get('superOverDetails'):
            # This also needs to be part of returned data, not printed for web
            print("--- Super Over Details ---"); [print(detail) for detail in res['superOverDetails']]; print("-------------------------")

        print(f"\nResult: {res['winMsg'].upper()}") # For CLI, will be returned for web
        print(random.choice(commentary_lines['end'])) # For CLI, will be part of commentary for web
        ...
        for bat_map in ['innings1Battracker', 'innings2Battracker']:
            ...
            for player in tracker:
                if player not in current_batting_info: current_batting_info[player] = copy.deepcopy(tracker[player]); current_batting_info[player]['innings'] = 1; current_batting_info[player]['scoresArray'] = [int(tracker[player]['runs'])]
                else: current_batting_info[player]['balls'] += tracker[player]['balls']; current_batting_info[player]['runs'] += tracker[player]['runs']; current_batting_info[player]['ballLog'] += tracker[player]['ballLog']; current_batting_info[player]['innings'] += 1; current_batting_info[player]['scoresArray'].append(int(tracker[player]['runs']))
        for bowl_map in ['innings1Bowltracker', 'innings2Bowltracker']:
            tracker = res[bowl_map]
            for player in tracker:
                if player not in current_bowling_info: current_bowling_info[player] = copy.deepcopy(tracker[player]); current_bowling_info[player]['matches'] = 1
                else: current_bowling_info[player]['balls'] += tracker[player]['balls']; current_bowling_info[player]['runs'] += tracker[player]['runs']; current_bowling_info[player]['ballLog'] += tracker[player]['ballLog']; current_bowling_info[player]['wickets'] += tracker[player]['wickets']; current_bowling_info[player]['noballs'] = current_bowling_info[player].get('noballs', 0) + tracker[player].get('noballs', 0); current_bowling_info[player]['matches'] += 1

        input("Press Enter to continue...") # To be removed for web
        # MODIFIED: The resList (containing logs, trackers etc.) should also be returned from playoffs
        return winner, loser, current_batting_info, current_bowling_info, res # Added res
    except Exception as e:
        print(f"Error during {matchtag.upper()}: {str(e)}"); input("Press Enter to continue or Ctrl+C to exit...")
        return team1, team2, current_batting_info, current_bowling_info, None # Return None for res on error
...
if __name__ == "__main__":
    ...
    all_match_results_and_commentary = [] # Store results for potential later use
    ...
    for team1, team2 in scheduled_matches_final:
        ...
        print(f"\nMatch: {team1.upper()} vs {team2.upper()}") # CLI
        try:
            input("Press Enter to start the match...") # CLI
            print(random.choice(commentary_lines['start'])) # CLI (match start)

            resList = game(False, team1, team2) # game() assumed to return detailed logs

            # MODIFIED: Get commentary, don't call display_ball_by_ball directly
            innings1_commentary = get_match_commentary(resList['innings1Log'])
            innings2_commentary = get_match_commentary(resList['innings2Log'])

            # Store or process commentary (e.g., for web display)
            # For now, printing a sample for verification
            print("Sample Innings 1 Commentary (first 3 lines):")
            for line in innings1_commentary[:3]: print(line)
            print("Sample Innings 2 Commentary (first 3 lines):")
            for line in innings2_commentary[:3]: print(line)

            # Store full resList with commentary for this match
            resList['innings1_full_commentary'] = innings1_commentary
            resList['innings2_full_commentary'] = innings2_commentary
            all_match_results_and_commentary.append(resList)

            if resList.get('superOverPlayed', False) and resList.get('superOverDetails'):
                print("--- Super Over Details ---"); [print(detail) for detail in resList['superOverDetails']]; print("-------------------------") # CLI
            print(f"\nResult: {resList['winMsg']}"); print(random.choice(commentary_lines['end'])) # CLI (match end)
            ...
            if winner_status == "cancelled":
                ...
                print(f"Match {team1.upper()} vs {team2.upper()} cancelled. Points shared.") # CLI
            ...
            print("\n(Refactored) Current Points Table Data:") # CLI
            for row in current_points_table: print(row)  # CLI

            top_players_stats = get_top_players(battingInfo, bowlingInfo)
            print("\n(Refactored) Top Players Data:") # CLI
            print(top_players_stats) # CLI
    ...
    final_points_table = get_points_table(points, TEAMS)
    print("\n(Refactored) Final Points Table Data:") # CLI
    for row in final_points_table: print(row) # CLI
    ...
    playoff_match_results = [] # Store playoff results
    ...
    if len(pointsTabulate) >= 4:
        ...
        if q1_res: playoff_match_results.append(q1_res)
        ...
        if len(finalists) == 2:
            ...
            if final_res: playoff_match_results.append(final_res)
            print(f"\n🏆 {finalWinner.upper()} WINS THE IPL!!!") # CLI
        else: print("Could not determine two finalists for the Final match.") # CLI
    else: print("Not enough teams qualified for playoffs.") # CLI

    # batStats.txt and bowlStats.txt writing will be removed in Step 6
    battingTabulate = []
