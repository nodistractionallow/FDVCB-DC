...
# dir_path related logic will be removed later or handled by the web application.
...
# battingf & bowlingf will be removed in cleanup as they are not part of the core returned data.
battingf = 0
bowlingf = 0
...
def run_league_match(team1, team2, is_rain_match, current_points, current_batting_info, current_bowling_info, web_mode=True):
    # web_mode=True will suppress CLI prints/inputs
...
def playoffs_match_simulator(team1, team2, matchtag, current_batting_info, current_bowling_info, web_mode=True): # Renamed from 'playoffs' to avoid conflict with future high-level playoff orchestrator
...
def simulate_tournament_league_phase(team_list, web_mode=True):
    ...
    global battingf, bowlingf # Temporary for CLI stat
    battingf, bowlingf = 0,0
    ...
    for team1_sched, team2_sched in scheduled_matches:
        ...
        if resList_match:
            ...
            if not web_mode: # Update CLI specific stats if not in web_mode
        ...
        if not web_mode: # CLI display updates
...
def simulate_tournament_playoffs(points_data, batting_info_data, bowling_info_data, team_list_arg, web_mode=True):
    ...
    for team_name_key in team_list_arg:
        team_data = points_data[team_name_key] # Assume all teams in team_list_arg are in points_data
        nrr_val = 0
    ...
    if len(ranked_teams) >= 4:
        q1_teams = [ranked_teams[0][0], ranked_teams[1][0]]
        elim_teams = [ranked_teams[2][0], ranked_teams[3][0]]
        finalists = []
        ...
        if q1_res: all_playoff_match_results.append(q1_res)
        ...
        if q2_res: all_playoff_match_results.append(q2_res)
...
if __name__ == "__main__":
    # Set web_mode=False for CLI prints, True to suppress them for web use
    is_web_mode = False
    ...
    if not is_web_mode:
        # Final stats display (using existing tabulate logic for CLI)
        # This part will be removed in Step 6 for web integration.
        final_batting_stats_display = []
        for b, c_stats in battingInfo.items():
            ...
            final_batting_stats_display.append([b, c_stats['innings'], c_stats['runs'], avg, max(c_stats['scoresArray']) if c_stats['scoresArray'] else 0, sr, c_stats['balls']])
        final_batting_stats_display = sorted(final_batting_stats_display, key=lambda x: x[2], reverse=True)

        final_bowling_stats_display = []
        for b, c_stats in bowlingInfo.items():
            ...
            final_bowling_stats_display.append([b, c_stats['wickets'], overs, c_stats['runs'], noballs, economy])
        final_bowling_stats_display = sorted(final_bowling_stats_display, key=lambda x: x[1], reverse=True)

        # The following stdout redirections and file writings will be removed in a later step.
        # For now, they write to files if not in web_mode.
        with open(os.path.join(dir_path, "batStats.txt"), "w") as f: sys.stdout = f; print(tabulate(final_batting_stats_display, headers=["Player", "Innings", "Runs", "Average", "Highest", "SR", "Balls"], tablefmt="grid")); sys.stdout = sys.__stdout__
        with open(os.path.join(dir_path, "bowlStats.txt"), "w") as f: sys.stdout = f; print(tabulate(final_bowling_stats_display, headers=["Player", "Wickets", "Overs", "Runs Conceded", "NB", "Economy"], tablefmt="grid")); sys.stdout = sys.__stdout__
