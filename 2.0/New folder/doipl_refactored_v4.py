...
def get_top_players(batting_info_data, bowling_info_data): # MODIFIED: Was display_top_players
    top_batsmen_list = []
    for player_name, stats in batting_info_data.items():
        outs = sum(1 for ball_log_entry in stats['ballLog'] if "W" in ball_log_entry) # Ensure correct key for ballLog
        avg = round(stats['runs'] / outs, 2) if outs else float('inf') # Use 'inf' for undefined average
    ...
    for player_name, stats in bowling_info_data.items():
        economy = round((stats['runs'] / stats['balls']) * 6, 2) if stats['balls'] else float('inf') # Use 'inf' for undefined economy
        top_bowlers_list.append({"Player": player_name, "Wickets": stats['wickets'], "Economy": economy})

    top_bowlers_list = sorted(top_bowlers_list, key=lambda x: (x["Wickets"], -x["Economy"]), reverse=True)[:3] # Sort by wickets desc, then economy asc
...
if __name__ == "__main__":
    ...
    for team1, team2 in scheduled_matches_final:
        ...
        try:
            ...
            # MODIFIED: Call get_top_players and print its results (temporary for verification)
